...
{	
	...
	pid_t pid;
	struct job_t *job;
	char *bgfg = argv[0]; /* bg, or fg command */
	char *id = argv[1];   /* pid, or jid */
	
	if (id[0] == '%') /* searching for jid */
	{
		jid = atoi(++id);
	}
	else jid = pid2jid(atoi(id));    /* if not a jid, it's the pid */
	
	job = getjobjid(jobs, jid);      /* get the job in question */
	pid = job->pid;                  
	kill(pid, SIGCONT);              /* send signal to job */
	
	if(strcmp("bg", bgfg) == 0) {   
		job->state = BG;
	}
	else if (strcmp("fg", bgfg) == 0) {
		job->state = FG;
		waitfg(pid);
	}
...
void sigchld_handler(int sig) 
{
	...
	while((pid = waitpid(-1, &stat, WUNTRACED|WNOHANG)) > 0) {
		if(WIFSTOPPED(stat)) {
			sigtstp_handler(WSTOPSIG(stat)); 
		} else if(WIFSIGNALED(stat)) {
			chld_sig = WTERMSIG(stat);
			if(chld_sig == SIGINT) 
